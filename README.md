...
```sh
curl -sfL https://github.com/powerman/dockerize/releases/download/v0.25.1/dockerize-v0.22.2-linux-amd64 | install /dev/stdin /usr/local/bin/dockerize
```
...
```sh
wget -O - https://github.com/powerman/dockerize/releases/download/v0.25.1/dockerize-v0.22.2-linux-amd64 | install /dev/stdin /usr/local/bin/dockerize
```
...
1. Use `powerman/dockerize` as base image -
   ...
   ```dockerfile
   FROM powerman/dockerize:0.25.1
   ...
...
2. Copy dockerize from the base image into your image
   ...
   ```dockerfile
   FROM powerman/dockerize:0.25.1 AS dockerize
   FROM node:18-slim
   ...
